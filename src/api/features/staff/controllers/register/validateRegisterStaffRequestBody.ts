...
export default function validateRegisterStaffRequestBody (data:registerStaffRequestBodyType) {
    return new Promise<validateRegisterStaffRequestBodyType>(async (resolve, reject)=> {
        ...
        if(!data.address) reject({ status: false, code: 422,  message:'Address field cannot be empty' });
        if(!data.city) reject({ status: false, code: 422,  message:'City field cannot be empty' });
        if(!data.state) reject({ status: false, code: 422,  message:'State field cannot be empty' });
        if(!data.zipCode) reject({ status: false, code: 422,  message:'Zip code field cannot be empty' });
