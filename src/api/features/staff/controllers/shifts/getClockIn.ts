...
import {
  sendAuthorizationFailureResponse,
...
import {
  getStaffRoleById,
  getStaffUserById,
} from "src/api/shared/services/db/staff.service";
...
export default function getClockIn(req: Request, res: Response) {
  ...
  if (validationResult.error)
    return sendValidationFailureResponse(res, validationResult.message);

  getStaffUserById(req.currentUser.id!)
    .then((foundStaff) => {
      if (!foundStaff)
        return sendNotFoundFailureResponse(res, "No record found for staff");
      ...
      fetchStaffShiftByDateAndStaffRef(foundStaff.id, requestDate)
        .then((foundStaffShift) => {
          if (!foundStaffShift)
            return sendNotFoundFailureResponse(
              res,
          ...
          const currentStartTime = {
            hr:
              parseInt(
                extractTimeFromISODateTime(req.body.startAt).split(":")[0]
              ) + 1,
            min: parseInt(
              extractTimeFromISODateTime(req.body.startAt).split(":")[1]
            ),
          };
          ...
          if (scheduleStartTime.hr > currentStartTime.hr)
            return sendAuthorizationFailureResponse(
              res,
              "Clock in request rejected, your shift hasn't begun"
            );
          if (scheduleStartTime.hr === currentStartTime.hr) {
            if (scheduleStartTime.min > currentStartTime.min)
              return sendAuthorizationFailureResponse(
                res,
                "Clock in request rejected, your shift hasn't begun"
              );
          ...
          if (scheduleEndTime.hr < currentStartTime.hr)
            return sendAuthorizationFailureResponse(
              res,
              "Clock in request rejected, your shift has ended"
            );
          if (scheduleEndTime.hr === currentStartTime.hr) {
            if (scheduleEndTime.min < currentStartTime.min)
              return sendAuthorizationFailureResponse(
                res,
                "Clock in request rejected, your shift has ended"
              );
          ...
          clockStaffInByObjId(foundStaff!.id)
            .then(async (clockedInStaff) => {
              const staffRole = await getStaffRoleById(
                clockedInStaff.providerRole
              );
            ...
            .catch((error) => {
              ...
              return sendServerFailureResponse(
                res,
                "There was an error clocking staff in"
              );
            });
        })
        .catch((error) => {
...
function validateGetClockInRequestBody(data: IGetClockInRequestBody) {
  if (!Object.keys(data).length)
    return { error: true, message: "Please send in the required data" };
  if (!data.startAt)
    return { error: true, message: "'StartAt' field cannot be empty" };
