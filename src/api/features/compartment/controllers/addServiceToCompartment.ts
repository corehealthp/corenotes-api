...
export default function addServiceToCompartment(req: Request, res: Response) {
  getCompartmentByCompartmentId(req.body.compartmentId).then(
    (foundCompartment) => {
      const subCompartments = foundCompartment?.subCompartments.flatMap(
        (subCompartment) => {
          return subCompartment._id;
        }
      );
      if (subCompartments?.includes(req.body.serviceId)) {
        const conflictError = new ConflictError(
          "Services has already been added to compartment"
        );
