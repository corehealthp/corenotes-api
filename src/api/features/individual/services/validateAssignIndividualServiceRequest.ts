...
export default function validateAssignIndividualServiceRequest(
  ...
  return new Promise<IAssignServiceToIndividualReqBody>((resolve, reject) => {
    ...
    if (!data.individualId)
      reject({ code: 422, message: "Individual id parameter cannot be empty" });

    if (!data.serviceId)
      reject({ code: 422, message: "Service id field cannot be empty" });
    if (!data.staffRole)
      reject({ code: 422, message: "Staff Role field cannot be empty" });

    getServiceByObjectId(data.serviceId).then((foundService) => {
      if (foundService) {
      ...
      if (
        foundService?.title.toLowerCase().split(" ").join("-") !==
        "medication-administration"
      ) {
        if (!data.schedule.startDate)
          reject({
            code: 422,
            message: "Service start date field cannot be empty",
          });

        if (!data.schedule.time)
          reject({ code: 422, message: "Service time field cannot be empty" });

        if (!data.schedule.frequency)
          reject({
            code: 422,
            message: "Service frequency type field cannot be empty",
          });
        ...
        if (
          ...
          if (!data.schedule.frequencyAttr)
            reject({
              code: 422,
              message:
                "Please select frequency value date field cannot be empty",
            });
        }
      }
    });
