...
export default async function logIn(req: Request, res: Response) {
  validateLoginRequestBody(req.body)
    .then(({ requestBody }) => {
      getStaffUserByUsername(requestBody.username.toLowerCase())
        .then((foundStaff) => {
          if (!foundStaff)
            return sendFailureResponse({
              ...
              statusCode: 401,
              message:
                "Oops! The username or password entered does not match our record. Please confirm and try again.",
            });
          ...
          verifyPassword(requestBody.password, foundStaff.password!)
                  ...
                  storeAuthToken(foundStaff.id, authToken)
                    .then((authenticatedUser) => {
                      return sendSuccessResponse({
                        res,
                        statusCode: 200,
                        message: "Staff User signed in successfully",
                        data: { user: authenticatedUser },
                      });
                    })
                    .catch((error) => {
                      console.log(
                        `AUTHENTICATION ERROR: There was an error generating authentication token, try logging in`
                      );
                      console.log(error);
                      sendFailureResponse({
                        res,
                        statusCode: 422,
                        message: "There was an error verifing credentials",
                      });
                    });
                })
                .catch((error) => {
            ...
            .catch((error) => {
              console.log(
                `AUTHENTICATION ERROR: There was an error verifying password, try to log in`
              );
              console.log(error);
              sendFailureResponse({
                res,
    ...
    .catch((error) => {
      ...
      console.log(
        `VALIDATION ERROR: There was an error validating login request body`
      );
      console.log(error);
      sendFailureResponse({
        res,
