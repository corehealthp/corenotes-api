...
export default function validateRegisterIndividual(
  data: registerIndividualRequestBodyType
) {
  return new Promise<validateRegisterIndividualRequestBodyType>(
    async (resolve, reject) => {
      ...
      if (!data.contact?.name)
        reject({
          status: false,
          code: 422,
          message: "Contact name field cannot be empty",
        });
      ...
      if (!data.contact?.phoneNumber)
        reject({
          status: false,
          code: 422,
          message: "Contact phone number field cannot be empty",
        });
      ...
      if (!data.codeAlert.length)
        reject({
          status: false,
          code: 422,
          message: "Code alert field cannot be empty",
        });
      ...
      if (!data.requestedServices?.length)
        reject({
          status: false,
          code: 422,
          message: "Requested services field cannot be empty",
        });
      ...
      if (!data.allergies?.other)
        reject({
          status: false,
          code: 422,
          message: "Other allergies field cannot be empty",
        });
